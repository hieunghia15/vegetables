...
class FarmProductTypeFilterController extends Controller
{

    public function  __construct(Ward $wardModel,District $distrctModel,Farmer $farmerModel,Province $provinceModel,User $userModel, Product $productModel, Express $expressModel,FarmProductType $farmProductTypeModel)   {
        $this->user=$userModel;
    ...
    public function show($product_type_slug)
    {
        ...
        $get_product=$this->products->all();
        $getProduct=$this->products->limit(3)->get();
        $product_Types=$this->farm_product_types->all();

        $productTypes = $this->farm_product_types->where('product_type_slug', $product_type_slug)->first()->id;

        $productname = $this->products->where('farm_product_type_id', $productTypes)->get();

        $product_news = $this->products->orderBy('id', 'desc')->limit(4)->get();

        return view('guest.product.farm-product-type-filter', compact('product_Types', 'productname','getProduct','provinces','get_product','productTypes','express','provinces', 'product_news'));
    }
    ...
    public function filter(Request $request){
        ...
        $productType=$request->get('product');
        ...
        if($expre==null && $province==null && $toplevel==null && $lastlevel==null){
            $get_product=$this->products
            ->where('farm_product_type_id',$productType)
            ->get();
        }
    ...
    public function filter_two(Request $request){
        ...
        if($chosen==0){
            $get_product=$this->products->orderBy('price','DESC')->get();
        }
        else{
            $get_product=$this->products->orderBy('price','asc')->get();
        }
