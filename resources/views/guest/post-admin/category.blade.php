...
<head>
    @include('guest.layout.head', [
        'title' => __($title ?? 'Trang thông tin chính thức của Nông sản việt'),
    ])
    @stack('head')
            ...
            <div class="row">
                <div class="post-tag">
                    ...
                    @foreach ($categories as $category)
                        <span class="pr-5"><button class="post-btn"
                                onclick="location.href='{{ route('guest.news.category-post', $category->category_slug) }}'">{{ $category->name }}</button></span>
                    @endforeach
                </div>
            </div>
            <div class="row">
